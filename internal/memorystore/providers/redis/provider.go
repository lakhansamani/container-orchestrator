...
func NewRedisProvider(redisURL string) (*memoryStoreProvider, error) {
	...
	if len(redisURLHostPortsList) > 1 {
		...
		if err != nil {
			log.Debug().Err(err).Msg("error connecting to redis")
			return nil, err
		}
